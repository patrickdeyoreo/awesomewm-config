...
-- localize the clock
--os.setlocale(os.getenv("LANG") or "POSIX")
...
local batspr_l = wibox.widget.imagebox(theme.w6)
local batspr_r = wibox.widget.imagebox(theme.w7)
...
local batbg = wibox.container.background(batbar, "#808080", gears.shape.rectangle)
...
local battery_widget1 = wibox.container.background(wibox.container.margin(
wibox.widget {
  bat_icon, bat_widget,  layout = wibox.layout.align.horizontal
}, 1, 1), theme.gray)
local battery_widget2 = wibox.container.background(wibox.container.margin(
wibox.widget {
  batspr_l, bat.widget, batspr_r, layout = wibox.layout.align.horizontal
}, 0, 0), theme.gray)
...
local mem = lain.widget.mem({
    settings = function()
        ...
        theme.fg_widget,
        --" " .. mem_now.used .. "MB [" .. mem_now.perc .. "%] "
        " " .. mem_now.perc .. "% "
        )))
    end
})
local mem_widget =  wibox.container.background(wibox.container.margin(
wibox.widget {
  mem_icon, mem.widget, layout = wibox.layout.align.horizontal
}, 0, 0), theme.pink)
...
local cpu_widget =  wibox.container.background(wibox.container.margin(
wibox.widget {
  cpu_icon, cpu.widget, layout = wibox.layout.align.horizontal
}, 0, 0), theme.pink)
...
local temp_widget =  wibox.container.background(wibox.container.margin(
wibox.widget {
  temp_icon, temp.widget, layout = wibox.layout.align.horizontal
}, 0, 0), theme.yellow)
...
local fs_widget =  wibox.container.background(wibox.container.margin(
wibox.widget {
...
theme.volume = lain.widget.alsabar({
    width = 100, border_width = 1, ticks = true, ticks_size = 10,
...
theme.volume.bar:buttons(awful.util.table.join (
          ...
          awful.button({}, 3, function()
            ...
            os.execute(string.format("%s set %s on", theme.volume.cmd, "Speaker"))
            theme.volume.update()
          end),
...
local volumebg = wibox.container.background(
  theme.volume.bar, "#808080", gears.shape.rectangle)
local vol_widget = wibox.container.margin(
  volumebg, 2, 7, 4, 4)
local volume_widget = wibox.container.background(wibox.container.margin(
wibox.widget {
  vol_icon, vol_widget, layout = wibox.layout.align.horizontal
}, 0, 0), theme.gray)
